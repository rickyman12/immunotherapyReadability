...
import subprocess
import collections
from readScores.langdata import LANGDATA
if sys.version[0] >= '3':
...
def getmeasures(text, lang='en', merge=False):
	...
	readScores = collections.OrderedDict([
	...
	if basicwords:
		...
		readScores['DaleChallIndex'] = DaleChallIndex(
				words, complex_words_dc, sentences)
	if merge:
		readScores.update(stats)
		readScores.update(wordusage)
		readScores.update(beginnings)
		return readScores
	return collections.OrderedDict([
			('readScores grades', readScores),
			('sentence info', stats),
...
def getdataframe(filenames, lang='en', encoding='utf8', tokenizer=None):
	"""Return a pandas DataFrame with readScores measures for a list of files.
	"""
	import pandas
